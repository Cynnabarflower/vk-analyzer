...
def show_graph(figure):
    ...
    """
    n = figure.number
    # closing all figures except the needed one
    for i in range(1, 4):
        if i != n:
            plt.close(i)
    plt.show()
...
def stkol(dframe):
    ...
    :return: data frame
    """
    dframe.drop(columns='sex')
    # Selecting all columns with numeric values
    # and creating a new data frame with the statistics
    m = dframe.select_dtypes(include='number').describe().transpose()
    m.columns = ['Количество данных', 'Арифметическое среднее', 'Стандартное отклонение',
                 'Минимум', 'Квартиль 1', 'Квартиль 2', 'Квартиль 3', 'Максимум']
    return m
