...
class ScrollList(tk.Frame):
    def __init__(self, parent, w=BUTTON_WIDTH + 2 * PADDING, h=(BUTTON_HEIGHT + PADDING) * 4 + PADDING / 2,
        ...
        self.padding = (padding, padding, padding, padding) if isinstance(padding, int) or isinstance(padding, float) else (
    ...
    def initcanvas(self):
        ...
        self.canvas.create_rectangle(self.padding[3], 0, self.w - self.padding[1], self.padding[0], outline=self.backgroundcolor,
    ...
    def updatecanvas(self):
        y = self.dy + self.item_padding + self.padding[0]
        for button in self.buttons:
            if self.padding[0] >= button.y >= self.padding[2] and self.padding[0] >= button.old_y >= self.padding[2]:
                continue
            if not button.needsupdate:
                itemparts = self.canvas.find_withtag((button.tag))
                for part in itemparts:
                    self.canvas.move(part, 0, y - button.y)
    ...
    def draw_new(self, button, y):
        ...
        if button.progress > 0.02:
            round_rectangle(self.canvas, x0 + self.progress_offset, y, (self.item_width - self.progress_offset) * button.progress + x0  + self.progress_offset, y + self.item_height,
                            radius=self.borderraadius, outline=self.loadcolor, fill=self.loadcolor,
                            tag=button.tag + 'progress')
        elif button.progress > 0:
            self.canvas.create_rectangle(x0 + self.progress_offset, y, (self.item_width - self.progress_offset) * button.progress + x0  + self.progress_offset, y + self.item_height, outline=self.loadcolor, fill=self.loadcolor,
                            tag=button.tag + 'progress')
    ...
    def sort(self, key=lambda item: item.progress, reverse=True):
...
class ProgressButton(tk.Frame):
    ...
    def drawProgress(self, prorgess):
        ...
        self.canvas.delete('progress')
        if prorgess > 0:
            round_rectangle(self.canvas, self.padding, self.padding, (self.w - self.padding * 2) * prorgess + self.padding,
                            self.h - self.padding,
                            radius=self.borderradius, outline=self.loadcolor, fill=self.loadcolor, tag='progress')
        if prorgess < 1:
            self.canvas.create_text(self.w / 2, self.h / 2, text=str(math.floor(prorgess * 100)) + '%', fill=self.textcolor,
...
class SimpleButton(tk.Frame):
    ...
    def __init__(self, parent, w=BUTTON_WIDTH + 2 * PADDING, h=BUTTON_HEIGHT + 2 * PADDING, backgroundcolor='#f1f0ec',
                 onclicked=None, text='+', textcolor='#ffffff',
                 fillcolor="#4978a6", loadcolor='#224b79', borderradius=18, padding=10, font=("Colibri", 25), fixed = False):
        tk.Frame.__init__(self, parent)
        self.w = w
        self.h = h
        self.text = text
